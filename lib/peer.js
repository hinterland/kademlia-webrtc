...
DataConnection.prototype.close = function() {
  ...
  this.emit('close');
  for(var i = 0; i < this.provider.connections[this.peer].length; i ++){
    if(this.provider.connections[this.peer][i].id === this.id){
      this.provider.connections[this.peer].splice(i, 1);
      break;
    }
  }
}
...
Negotiator.cleanup = function(connection) {
  ...
  if (!!pc && (pc.readyState !== 'closed' || pc.signalingState !== 'closed')) {
    pc.close();
    // remove listeners
    pc.onaddstream = pc.ondatachannel = pc.onnegotiationneeded = null;
    pc.onicecandidate = pc.oniceconnectionstatechange = pc.onicechange = null;
    // remove from the navigator
    Negotiator.pcs[connection.type][connection.peer][pc.id] = {};
    delete Negotiator.pcs[connection.type][connection.peer][pc.id];
  }
}
